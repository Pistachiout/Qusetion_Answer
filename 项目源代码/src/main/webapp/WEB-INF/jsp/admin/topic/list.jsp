...
<div class="content-wrapper" style="padding: 50px 0 40px;">
    ...
    <ol class="breadcrumb">
      ...
      <li><a href="/admin/topic/list">问题</a></li>
          ...
          <c:forEach var="topic" items="${page.list}">
            <tr>
              ...
              <td>
                ...
                <c:if test="${!topic.showStatus}">屏蔽中</c:if>
              </td>
              ...
              <td>
                  <shiro:hasPermission name="topic:top">
                  	<button onclick="actionBtn('${topic.topicId}', 'top', this)" class="btn btn-xs btn-primary">
                  		<c:choose>
                  			<c:when test="${!topic.showStatus}">取消屏蔽</c:when>
                  			<c:otherwise>屏蔽</c:otherwise>
                  		</c:choose>
                  	</button>
                  </shiro:hasPermission>
                 <%-- <shiro:hasPermission name="topic:good">
                   ...
                   <shiro:hasPermission name="topic:good">
                     <button onclick="actionBtn('${topic.topicId}', 'block', this)" class="btn btn-xs btn-primary">
                       <c:choose>

                         <c:when test="${topic.showStatus}">
                           <a href="/admin/topic/block?id=${topic.topicId}"  class="btn btn-xs btn-primary">屏蔽</a>
                         </c:when>
                         <c:otherwise>
                           <a href="/admin/topic/block?id=${topic.topicId}" class="btn btn-xs btn-primary">取消屏蔽</a>
                         </c:otherwise>
                       </c:choose>
                     </button>
                   </shiro:hasPermission>--%>
